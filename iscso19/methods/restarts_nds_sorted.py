...
def solve(seed):
    ...
    method = MyGeneticAlgorithm(
        ...
        callback=MyCallback(folder, name="restarts_nds_sorted"),
    ...
    np.savetxt(os.path.join(folder, f"restarts_nds_sorted_{seed}.x"), res.pop.get("X").astype(np.int))
    np.savetxt(os.path.join(folder, f"restarts_nds_sorted_{seed}.f"), res.pop.get("F"))
    np.savetxt(os.path.join(folder, f"restarts_nds_sorted_{seed}.g"), res.pop.get("G"))
